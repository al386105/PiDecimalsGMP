 ...
 */
void BBPIteration(mpf_t pi, int n, int dep_a, mpf_t dep_m, 
                mpf_t quot_a, mpf_t quot_b, mpf_t quot_c, mpf_t quot_d, mpf_t aux){
    mpf_set_ui(quot_a, 4);                  //  4 / (8n + 1)
    mpf_set_ui(quot_b, 2);                  // -2 / (8n + 4)
    mpf_set_ui(quot_c, 1);                  // -1 / (8n + 5)
    mpf_set_ui(quot_d, 1);                  // -1 / (8n + 6)
    mpf_set_ui(aux, 0);                     // a + b + c + d  

    mpf_div_ui(quot_a, quot_a, dep_a + 1);  // 4 / (8n + 1)
    mpf_div_ui(quot_b, quot_b, dep_a + 4);  // 2 / (8n + 4)
    mpf_div_ui(quot_c, quot_c, dep_a + 5);  // 1 / (8n + 5)
    mpf_div_ui(quot_d, quot_d, dep_a + 6);  // 1 / (8n + 6)
...
void SequentialBBPAlgorithm(mpf_t pi, int num_iterations){   
    ...
    for(i = 0; i < num_iterations; i++){ 
        BBPIteration(pi, i, dep_a, dep_m, quot_a, quot_b, quot_c, quot_d, aux);   
        // Update dependencies:  
    ...
    #pragma omp parallel 
    {
        ...
        #pragma omp parallel for 
            for(i = thread_id; i < num_iterations; i+=num_threads){
                BBPIteration(local_pi, i, dep_a, dep_m, quot_a, quot_b, quot_c, quot_d, aux);
                // Update dependencies:  
                mpf_mul(dep_m, dep_m, jump);
                dep_a += num_threads << 3;  // dep_a += num_threads * 8
            }
